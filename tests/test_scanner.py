...
def make_slot(start_time, available_players, booking_list=None, course_id=1):
    return {
        ...
        "availableParticipantNo": available_players,
...
class TestFindMatchingSlots:
    ...
    def test_no_course_filter_matches_any_course(self):
        ...
        slots = [make_slot("2026-06-15T09:00:00", [4], course_id=999)]
        assert find_matching_slots(slots, cfg) == slots

    def test_multiple_matching_slots_all_returned(self):
        cfg = make_cfg()
        slots = [
            make_slot("2026-06-15T08:00:00", [4]),
            make_slot("2026-06-15T09:00:00", [4]),
            make_slot("2026-06-15T10:30:00", [4]),  # outside window
        ]
    ...
    def test_fractional_time_window(self):
        ...
        slots = [
            make_slot("2026-06-15T08:29:00", [4]),  # < 8.5
            make_slot("2026-06-15T08:30:00", [4]),  # == 8.5, included
            make_slot("2026-06-15T09:30:00", [4]),  # == 9.5, excluded
        ]
        result = find_matching_slots(slots, cfg)
        assert len(result) == 1
    ...
    def test_mixed_valid_invalid_slots(self):
        ...
        slots = [
            make_slot("2026-06-15T09:00:00", [4], course_id=1),   # match
            make_slot("2026-06-15T09:00:00", [3], course_id=1),   # wrong players
            make_slot("2026-06-15T09:00:00", [4], course_id=2),   # wrong course
            make_slot("2026-06-15T09:00:00", [4], course_id=1, booking_list=[{}]),  # booked
        ]
        result = find_matching_slots(slots, cfg)
        assert len(result) == 1
